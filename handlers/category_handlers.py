...
async def search_category(message: Message):
    keyword = message.text.lower()
    cyrillic_to_latin = {
        'а': 'a', 'б': 'b', 'в': 'v', 'г': 'g', 'д': 'd', 'е': 'e', 'ё': 'yo', 'ж': 'j', 'з': 'z', 'и': 'i',
        'й': 'y', 'к': 'k', 'л': 'l', 'м': 'm', 'н': 'n', 'о': 'o', 'п': 'p', 'р': 'r', 'с': 's', 'т': 't',
        'у': 'u', 'ф': 'f', 'х': 'x', 'ц': 'c', 'ч': 'ch', 'ш': 'sh', 'щ': 'sch', 'ъ': '', 'ы': 'y', 'ь': '',
        'э': 'e', 'ю': 'yu', 'я': 'ya'
    }
    latin_keyword = ''.join(cyrillic_to_latin.get(char, char) for char in keyword)

    products = db.get_products_by_category(keyword) or db.get_products_by_category(latin_keyword)

    if products:
        await message.answer("Products found in this category:")
        for product in products:
            await message.answer(f"Name: {product['name']}, Price: {product['price']}")
    else:
        await message.answer("No products found in this category.")
